class GlyphModel {
  ...
  // hashmap to keep attributes names and values
   ...
   * The length of input array of floats must be equal to the number of attributes.
   */
   ...
   */
  String getAttrName(int attrIndex) {
    return attrsNames[attrIndex];
  }

  /**
   * Return the value of an attribute.
   */
  Float getAttrValue(int attrIndex) {
    return attrs.get(getAttrName(attrIndex));
  }
  ...
  private void drawLinePattern(PGraphics pg, float n_lines, float dim) {
  ...
  private void drawFigure(PGraphics pg, float x, float y, float n_lines, float dim) {
    float n = ceil(n_lines*10);
    float rad = dim*0.5;
  ...
  private void star(PGraphics pg, float x, float y, float radius1, float radius2, boolean curve) {
  ...
  private boolean useAttr(String attrName) {
    for (String k : attrs.keySet()) {
      if (k.contains(attrName) && attrs.get(k) != null) {
        return true;
      }
    }
    return false;
